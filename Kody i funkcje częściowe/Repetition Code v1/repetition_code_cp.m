...
cp = 10;      %prawdopodobieñstwo przek³amania w %
...
disp(codeWord);
%----------KANA£----------
codeWordwNoise = 1 : n * length(message); %tablica codeWordwNoise - s³owo kodowe po przejœciu przez kana³

for i = 1 : length(codeWord) 
  e = randi ([0 100],1,1) %generowanie losowej liczby z zakresu 0 - 100%
  if e > 100 - cp %wprowadzenie zak³óceñ
    if codeWord(i) == 1 %zamiana bitów na przeciwne
      codeWordwNoise(i) = 0
    else 
      codeWordwNoise(i) = 1
    endif
  else
    if codeWord(i) == 1 %brak zamiany bitów na przeciwne
      codeWordwNoise(i) = 1
    else
      codeWordwNoise(i) = 0
    endif
